...
ui <- fluidPage(
  ...
  sidebarPanel(
    ...
    selectInput("pcas", "PCAs", dist <- str_c("PC", 1:9), selected = "PC1", multiple = TRUE),
    selectInput("var3", "X axis Variable", str_c("PC", 1:5)),
    selectInput("var4", "Y axis Variable", str_c("PC", 1:5))
  ),
  mainPanel(
    HTML("<h3>Note: The wine with a quality greater than 6.5 is considered to be of good quality.</h3>"),
    fluidRow(
    ...
    column(12,h3("Principal Component Analysis"),plotOutput("PCA_component")),
    column(12,h3("PCA Scores Plot"),plotOutput("PCA_scores"))
  )
)
...
server <- function(input, output) {
  ...
  output$PCA_component <- renderPlot({
    ggplot(pca_result %>% filter(component %in% input$pcas)%>%mutate(terms = reorder_within(terms, abs(value), component))) +
