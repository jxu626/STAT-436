...
text2=p("Does the effectiveness of PM prevention and control follow the same trend in every country? Only countries with complete records from 1990 to 2019 were retained to ensure consistency with the countries selected in the Environmental Policy Stringency Index. The range of years selected is used to compare the top 10 countries with the earliest deaths caused by environmental particulate matter (PM) and the top 10 countries with the earliest deaths caused by PM per million inhabitants, separately.
")
...
Users can select one type of emission source and a time range to depict on the heat map below. The heat map shows the value for the selected emission type and the bar graph shows the average of all the emission types over the given time range. Note: There are some datapoints missing for certain types of CO2 emission sources for some years and countries.",
              ...
              tabPanel("Trend in Mortality Rate",
                       ...
                       sidebarLayout(
                         sidebarPanel(
                           text2,
                           sliderInput("year", "Year Range", min =2005, max = 2019, c(2005, 2019), sep = "")
                         ),
                         mainPanel(
                           tabsetPanel(
                             tabPanel("Visualize",
...
server <- function(input, output) {
  ...
  mortality_subset <- reactive({
    health %>%
      ...
      filter(Year == 1990 | Year == input$year[2])%>%
  ...
  output$ratings_scatter <- renderPlot({
    mortality_subset()%>%
      ...
      labs(x="Country", y="Number of Premature Death", title = "Top 10 Country with Most Premature Death Caused by Ambient Particulate Matter (PM) ")+
      theme_bw()+
  ...
  output$ratings_scatter3 <- renderPlot({
    mortality_subset2() %>%
      ...
      geom_text(aes(label=death_per_million), position=position_dodge(width=0.75), hjust=0.4, vjust=-0.65, size=3)+
      scale_y_continuous(expand = c(0, 0, 0.1, 0.1))+
      labs(x="Country", y="Premature Death per Million", title = "Top 10 Countries with Most Premature Death Per Million Inhabitant Caused by PM ")+
      theme_bw()+
      theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,size=15),
            axis.ticks = element_blank(),
            panel.grid.major.x = element_blank())
